  ...
  <body>
	...
	<main class="main" role="main">
		...
		<style type="text/css">
			...
			.collapseNumber{
				box-shadow: 2px 1px 14px 0px #00000042;
				...
				<!--- //Content -->
							...
							while($row = mysqli_fetch_array($respuesta22))
                            {
                            	if($row['nombre'] != $temp_categoria){
                            		...
                            		?>
							        <button style="font-size: 11px;width: 100%; margin-top: 5px;margin-bottom: 5px;" onclick="return toggleAccordion(<?php echo $collapse; ?>)" class="btn btn-accordion" data-toggle="collapse" data-target="#collapse<?php echo $collapse; ?>" aria-expanded="true" aria-controls="collapse<?php echo $collapse; ?>">
							...
							<?php
							if($tipo_traslado == 1 || $tipo_traslado == 4 ){
							?>
							<div style="margin-bottom: 10px" class="collapseNumber">
					          <button class="btn btn-accordion" style="font-size: 11px;" onclick="return showReturnProducts(this)">Click para agregar productos al retorno</button>
					        </div>
